                ...
                <div class="container mx-auto">
                    <form method="post" action="{{ route('student.update') }}" class="mt-6 space-y-6">
                                ...
                                <label>
                                    <select name="teacher" class="'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm'">
                                        ...
                                        @foreach($teachers as $teacher)
                                            <option {{old('teacher', $student->teacher_id) == $teacher->id ? 'selected' : ''}} value="{{$teacher->id}}">{{$teacher->name}}</option>
                                        @endforeach
                                    </select>
                                </label>
                            ...
                            <label>
                                <select name="parent" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                    ...
                                    @foreach($parents as $parent)
                                        <option {{old('parent', $student->parent_id) == $parent->id ? 'selected' : ''}} value="{{$parent->id}}">{{$parent->name}}</option>
                                    @endforeach
                                </select>
                            </label>
                        ...
                        <div>
                            ...
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $student->name)" autocomplete="name" />
                            ...
                            <label>
                                <select name="standard" class="'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm'">
                                    ...
                                    @foreach(range(1, 12) as $item)
                                        <option {{old('standard', $student->standard) == $item ? 'selected' : ''}} value="{{$item}}">{{$item}}</option>
                                    @endforeach
                                </select>
                            </label>
